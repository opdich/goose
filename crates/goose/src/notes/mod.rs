...
impl NoteGenerator {
    ...
    pub async fn generate_note_from_sessions(
            ...
            if let Some(conversation) = session.conversation {
                ...
                for message in conversation.messages() {
                    let message_id = message
                        .id
                        .clone()
                        .unwrap_or_else(|| format!("msg_{}", message.created));
        ...
        tracing::debug!(
            "Formatted conversation text: {} chars",
        ...
        let (response, _usage) = self
            .provider
            .complete(NOTE_GENERATION_SYSTEM_PROMPT, &[user_message], &[])
            .await
        ...
        let title = user_title.unwrap_or_else(|| {
            self.generate_title(&note_content)
                .unwrap_or_else(|_| "Untitled Note".to_string())
        });
...
pub async fn generate_note(
    ...
    generator
        .generate_note_from_sessions(session_ids, user_title)
        .await
}
